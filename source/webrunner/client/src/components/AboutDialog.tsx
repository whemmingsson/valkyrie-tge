...
const W = ({ children }: CCProps) => {
    return <span className="bg-red-800 p-1">{children}</span>;
}
    ...
    return (
        <dialog ref={ref} style={{ background: '#242424' }} className="w-2/3 h-1/2 border-2 border-gray-400 border-solid max-w-5xl" onClick={handleClick}>
            ...
            <div className="text-white p-4">
                <p className="pb-2 pt-2"><W>This engine and runner is under development. The developer takes no resposibility for anything at this point. You have been warned.</W> </p>
                <p className="mt-3 mb-3 pb-2 pt-2">A Valkyrie Game is a text based adventure where you progress in the game by typing commands.<br /> Some examples of commands are <CC>open door</CC>, <CC>turn left</CC> and <CC>inspect chest</CC></p>
